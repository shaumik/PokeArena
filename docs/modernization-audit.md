...
| Sky Drop                                                                                | Two-turn move where the user grabs the target. **"Doubles-only really" was wrong** — its target is `any`, the same as Fly, and eleven of upstream's nineteen cases are singles. Shipped; see `internal/engine/skydrop.go`. What it does *not* model is semi-invulnerability, which this engine has nowhere, and that is what the three terrain rows still want. |
