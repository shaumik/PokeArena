...
func run(upstreamDir, dataDir string, noSwap bool) error {
	...
	log.Printf("extracted %d species, %d moves, %d items, %d types, %d natures from snapshot (gen %d, @pkmn/sim %s)",
		len(up.Species), len(up.Moves), len(up.Items), len(up.Typechart), len(up.Natures), up.Meta.Gen, up.Meta.SimVersion)
