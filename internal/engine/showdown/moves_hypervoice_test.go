...
// Ported from test/sim/moves/hypervoice.js.
//
...
func TestMovesHyperVoice(t *testing.T) {
	describe(t, "Hyper Voice", func(g *psg) {
		g.it("should pierce through substitutes", func(p *ps) {
			...
			p.makeChoices("move hypervoice", "move rest")
			p.damaged(p.foe(), "Hyper Voice is a sound move and should have reached Caterpie itself")
			p.logLacks("substitute took the damage", "the Substitute should not have absorbed a sound move")
		})
	})

	describe(t, "Hyper Voice [Gen 5]", func(g *psg) {
