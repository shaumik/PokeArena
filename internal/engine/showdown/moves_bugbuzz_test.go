...
// Ported from test/sim/moves/bugbuzz.js.
//
...
func TestMovesBugBuzz(t *testing.T) {
	describe(t, "Bug Buzz", func(g *psg) {
		g.it("should pierce through substitutes", func(p *ps) {
			...
			p.makeChoices("move bugbuzz", "move rest")
			p.damaged(p.foe(), "Bug Buzz is a sound move and should have reached Caterpie itself")
			p.logLacks("substitute took the damage", "the Substitute should not have absorbed a sound move")
		})
	})

	describe(t, "Bug Buzz [Gen 5]", func(g *psg) {
