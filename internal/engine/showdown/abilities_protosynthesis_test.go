...
// measurement that multiplier would show: the same attack, at the same seed,
// under the condition being tested and under a control where the boost is
// definitely off or definitely on. A 1.3x gap clears every damage roll, so the
// comparisons hold on all seeds.
//
...
func TestAbilitiesProtosynthesis(t *testing.T) {
	...
	describe(t, "Protosynthesis", func(g *psg) {
		...
		g.it("should not activate while Desolate Land is active", func(p *ps) {
			...
			p.atLeast(inSun, underDesolateLand+1, "only real Sun should turn Protosynthesis on")
		})
		...
		g.it("should be activated by Booster Energy when Sunny Day is not active", func(p *ps) {
			...
			p.atLeast(bare, withBooster+1, "Booster Energy should hold the boost up once the Sun is gone")
		})
		...
		g.it("should not be prevented from activating if the user holds Utility Umbrella", func(p *ps) {
			...
			inRain := damageTaken(p, "move raindance", "move psychic")
			p.atLeast(inSun, 1, "the measured attack should have done damage")
			p.atLeast(inRain, inSun+1, "Utility Umbrella should not keep Protosynthesis from reading the Sun")
		})
		...
		g.it("should be deactiviated by weather suppressing abilities", func(p *ps) {
			...
			p.atLeast(inSun, 1, "the measured attack should have done damage")
			p.atLeast(suppressed, inSun+1, "Cloud Nine arriving should end the boost even though the Sun is still up")
		})
		...
		g.it("should not activate if weather is suppressed", func(p *ps) {
			...
			p.atLeast(suppressed, inSun+1, "Sun that starts under Cloud Nine should not turn Protosynthesis on")
		})
		...
		g.it("should activate when weather supression ends", func(p *ps) {
			...
			p.atLeast(suppressed, 1, "the measured attack should have done damage")
			p.atLeast(suppressed, unsuppressed+1, "the boost should start once the suppressor leaves")
		})
		...
		g.it("should have its boost nullified by Neutralizing Gas", func(p *ps) {
			...
			p.atLeast(nullified, boosted+1, "Neutralizing Gas should switch the boost off")
			p.atLeast(nullified, restored+1, "and the boost should come back when it leaves")
		})
