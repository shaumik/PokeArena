...
// is the named example. The plates now ship, but only as type boosters — the
// forme change they drive is the part that is missing, along with formes in
// general — and four of the six cases are gen 4 or gen 7 besides. Substituting
...
func TestMiscArceus(t *testing.T) {
	describe(t, "[Hackmons] Arceus", func(g *psg) {
		g.skip("in untyped forme should change its type to match the plate held",
			"Arceus is not in this 80-species dex, Multitype and plate-driven formes are not modeled, and this is gen 4 mechanics")
		g.skip("in Steel forme should should be Water-typed to match the held Splash Plate",
			"Arceus is not in this 80-species dex, plate-driven formes are not modeled, and this is gen 4 mechanics")
